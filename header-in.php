<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Iniciar sesión</title>
    <link href="stylesheets/style.css" rel="stylesheet" type="text/css">
    <link href="stylesheets/print.css" media="print" rel="stylesheet" type="text/css"/>
    <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css">
</head>
<body>
<header>
    <div class="contenedor">
        <div class="col-4">
            <h1 class="logo"><a href="#">atelier <span class="company">#SantoEncanto</span><span class="user">Bienvenida Jeannette!</span></a></h1>
        </div>
        <!--/col-4-->
        <nav class="col-8">
            <ul class="list-ppal">
                <li class="search"><a href="#"><i class="fa fa-search" aria-hidden="true"></a></i></li>
                <li><a href="inicio-sesion.php" target="_self">cerrar sesión</a></li>
            </ul>
    </div>
    </nav>
    <!-- /col-8 -->
    <div class="mobilenav">
        <nav>
            <ul>
                <li><a href="bride.php">novias</a></li>
                <li><a href="workshop.php">confecciones</a></li>
                <li><a href="file.php">archivador</a></li>
                <li><a href="#">balances</a></li>
                <!--<li><a href="#">descuentos</a></li>-->
                <li><a href="users.php">usuarios</a></li>
                <!--<li><a href="#">Contactos</a></li>-->
                <li><a href="#">cerrar sesión</a></li>
            </ul>
        </nav>
    </div>
    <a href="javascript:void(0)" class="icon">
        <div class="hamburger">
            <div class="menui top-menu"></div>
            <div class="menui mid-menu"></div>
            <div class="menui bottom-menu"></div>
        </div>
    </a>
    <!-- /content -->
</header>
